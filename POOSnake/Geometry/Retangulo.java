...
public class Retangulo extends Poligono {
    ...
    private void check(List<Ponto> pontos) {
        ...
        for (int i = 0; i < pontos.size(); i++) {
            if (produtoEscalar(pontos.get(i), pontos.get((i + 1) % pontos.size()),
                    pontos.get((i + 2) % pontos.size())) != 0) {
                System.out.println("Retangulo:vi");
                System.exit(0);
            }
        }
    ...
    /**
     * Rotaciona o retângulo em torno de um ponto (centroide) por um determinado
     * ângulo.
     * 
     * @param anguloGraus ângulo de rotação em graus.
     * @param centroide   ponto central em torno do qual o retângulo será
     *                    rotacionado.
